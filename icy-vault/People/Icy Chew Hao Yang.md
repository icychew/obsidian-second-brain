---
date: 2026-06-14
type: person
tags:
  - person
  - owner
ai-first: true
role: "Mechanical engineering undergraduate, builder, photographer and videographer"
company: "[[Icy Production]]"
relationship_strength: self
last_interaction: 2026-06-14
follow_up_date:
contact_email: haoyangchew2005@gmail.com
location: "Kuala Lumpur, Malaysia"
---

# Icy Chew Hao Yang

## For future Claude
The owner of the vault - the person you work for. This is the full profile that [[CRITICAL_FACTS]] summarizes. Pull it when reasoning about who Icy is, his background, ventures, and track record. Identity and operating style: [[SOUL]]; values: [[CORE_VALUES]].

## About
- 21 years old, born in Penang, now based in Kuala Lumpur, Malaysia.
- Year 2 Bachelor of Mechanical Engineering at Universiti Malaya (entered Sep 2024). See [[Mechanical Engineering at Universiti Malaya]].
- Concurrently doing the ACCA Foundation in Accountancy, at paper 5 as of 2026-06. See [[ACCA Foundation in Accountancy]].
- Builder across hardware and software; an active vibe coder with Claude. See [[Coding Projects Index]] and [[Vibe Coding with Claude]].
- Creative entrepreneur: runs [[Icy Production]] for photography and videography.

## What he cares about
- Building a one-person company as his north-star goal. See [[Build a One-Person Company]].
- Community and creative leadership through Photojam, the youth wing of Persatuan Fotografi Selangor dan KL. See [[Photojam]].
- Giving back and making things easier for others. See [[CORE_VALUES]].
- Building toward AI-driven engineering simulation (FEA / CAE / CFD) and, after graduating, a multinational engineer role. See [[AI-driven FEA CAE CFD]] and [[Work at a Multinational Engineer]].

## Track record
- Chief Editor and publisher lead for the Photojam book ([[Photojam Book]]), released 2026-03-01; 300+ of a 1000-copy / 5-year goal sold so far.
- Band manager for the Photojam original theme song and MV ([[Photojam Band and MV]]).
- Community service: graphic-design banners for hawkers at Teratai market with YB [[Yew Jia Haur]] ([[Teratai Market Banners]]).
- Camera-brand workshops with Fujifilm, Sony, and Nikon ([[Camera Brand Workshops]]).
- Released a single, 想你不只一天两天, through the Kuayue UM club on major music platforms ([[Kuayue UM Single - 想你不只一天两天]]).
- Earlier: digital marketing at Nildeco MY (2022, see [[Nildeco MY]]); the Elementum periodic-table game at matrikulasi (2023, see [[Elementum]]).
- Engineering and AI builds: [[Smart EV Battery Pack]], [[Smart City Bot]] (2nd runner-up), [[Pallet i]] (CREST), and [[EVi - EV Troubleshooting Chatbot]]. See [[Coding Projects Index]].
- Engineering leadership: Executive Committee of the [[UM Makers Club]] (runs Makerspace@UM; national events MakerXpreneur 2026 and ARC: RoboSurvivor 2026).

## How we can help each other
- He works at night - asynchronous, important-points-first communication suits him.
- Frame decisions from first principles with explicit trade-offs.
- He has no technical mentor and is fully self-taught, using AI as his main leverage - support that: explain the why, teach as you go, and help him build it himself.

## Notes
- Education, work, and travel history: [[Timeline]].

---

## Interactions

```dataview
LIST FROM "Daily"
WHERE contains(file.outlinks, this.file.link)
SORT date DESC
LIMIT 15
```
